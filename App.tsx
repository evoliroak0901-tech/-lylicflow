...
const cleanLyricsText = (text: string) => {
    return text
        ...
        .replace(/─/g, '')
        .replace(/━/g, '')
        // Remove standard brackets content (e.g., [Verse 1], [Chorus], (Bridge), 【Aメロ】, <Sabi>)
        .replace(/[\[【(<].*?[\]】)>]/g, '')
        // Remove empty lines created by removals and handle section labels without brackets
        .split('\n')
        .map(line => line.trim())
        .filter(line => {
            // Filter out lines that are just section labels (e.g., "Verse 1", "Chorus:")
            const sectionKeywords = ['verse', 'chorus', 'bridge', 'outro', 'intro', 'sabi', 'hook', 'aメロ', 'bメロ', 'サビ', 'soli', 'instrumental', '間奏', '前奏', '後奏'];
            const lowerLine = line.toLowerCase().replace(/[:：]/g, '').trim();
            const isSectionLabel = sectionKeywords.some(keyword => {
                // Check if the line is exactly the keyword OR keyword followed by a number
                const regex = new RegExp(`^${keyword}(\\s*\\d+)?$`, 'i');
                return regex.test(lowerLine);
            });
            return line !== '' && !isSectionLabel;
        })
        .join('\n');
};
